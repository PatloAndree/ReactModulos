    ...
    <div>
      {
            ...
            <tbody>
              {ventasFiltradas.map((venta) => (
                <tr key={venta.id}>
                  <td>{venta.id}</td>
                  <td>{venta.nombre}</td>
                  <td >{venta.tipo_venta == 1 ? "Alto" : venta.tipo_venta == 2 ? "Medio" : "Bajo" }</td>
                  <td>S/.{venta.monto}</td>
                  <td>S/.{venta.ganancia}</td>
                  <td>{(venta.created_at).split('T')[0]}</td>
                  {/* <td>{venta.updated_at}</td> */}
                </tr>
              ))}
            </tbody>
        ...
        :
        "No hay datos en las fechas o tipo seleccionado"
      }
    </div>
  );
};
